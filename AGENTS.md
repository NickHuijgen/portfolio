...
| The **mix of subjects** in `photos.yaml` (a tag used for the first time, or one whose share of the library changes materially) | `PERSON_LABELS.knowsAbout` (`src/lib/schema.ts`), `about.yaml`'s `homeIntro`/`body`/`facts`, and `homeTitle`/`homeDescription` (`src/lib/i18n.ts`) — see step 12b of the add-photos skill | These all state, in prose or in structured data, what this site's photographs are *of*, and nothing reads `photos.yaml` to check them — so they go quietly false rather than failing a build. `knowsAbout` shipped `Wildlife photography`/`Portrait photography` on every page for a while: `wildlife` was the old name of the `animals` tag and these are zoo animals, while `portrait` had exactly one photo behind it. Unlike every other row here this one is **not** a mechanical sync — it's the user's own voice and their search presence, so surface what's drifted and propose wording rather than rewriting it. |
